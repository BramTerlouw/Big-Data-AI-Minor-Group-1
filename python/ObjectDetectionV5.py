...
class ObjectDetectionV5:
    ...
    def draw_matrix(self, frame, frame_width, frame_height, distance_x, distance_y):
        # Draw boxes horizontally
        for i in range(math.ceil(frame_width / distance_x) + 1):
            cv2.rectangle(frame, (0, 0), (i * distance_x, distance_y), (255, 255, 0), 1)

            # Draw boxes vertically
            for j in range(math.ceil(frame_height / distance_y) + 1):
                cv2.rectangle(frame, (0, 0), (i * distance_x, j * distance_y), (255, 255, 0), 1)
    ...
    def plot_boxes_paddle(self, results, frame):
        ...
        for i in range(len(labels)):
            ...
            if self.score_is_sufficient(row[4], 0.2):
                ...
                cv2.putText(frame, self.class_to_label(labels[i]), (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.9,
                            (0, 255, 0), 1)
    ...
    def get_distance_between_human_and_paddle(self, frame, most_right_human, results_paddle):
        ...
        cv2.putText(frame, f"{distance_between_paddle_and_human} CM", (px2, py1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                    (255, 0, 0), 1)
    ...
    def generate_image(self):
        ...
        if most_right_human is not None:
            ...
            return self.summarize_results(frame, results_paddle, most_right_human, distance)

        # if we ur executing with PHP comment this out!
...
detection = ObjectDetectionV5(model_name='model/YOLOv5/model_paddle_V5.pt')
